...
3. ExamList와 동일하게 **포트 변경 여부(Y/N) → DB 계정 → DB 비밀번호**만 묻습니다. 포트 변경에 Y를 선택한 경우에만 포트번호를 입력하며, N 또는 Enter는 ApplyHub 기본 포트 `3000`을 사용합니다. DB 주소 `127.0.0.1:3306`과 DB 이름 `applyhub`는 자동 설정합니다. 계정과 비밀번호는 필수이며, 기존 값이 있으면 Enter로 유지합니다. 비밀번호 입력은 화면에 표시되지 않습니다.
...
설정이 완료된 PC에서는 `start-server.bat` 실행 시 다시 질문하지 않습니다. 기존 `.env`의 이메일 등 다른 설정은 유지합니다. 초기 설정을 다시 실행하려면 `start-server.bat --setup`을 사용합니다. 이때 DB 주소·포트·이름은 위 기본값으로 다시 설정됩니다. 외부 DB 등 다른 접속정보가 필요하면 초기 설정 이후 `.env`를 직접 수정하세요. 기존 서버의 데이터를 옮기는 경우에는 DB와 업로드 파일도 복원해야 합니다.

이미 `.env`에 `DB_NAME=admitcard`가 설정되어 있다면 일반 실행과 업데이트는 해당 DB를 사용합니다. `--setup`을 다시 실행하면 DB 이름이 `applyhub`로 설정됩니다. 기존 데이터가 필요하다면 먼저 `applyhub` DB로 데이터를 이전해야 하며, 설정 변경만으로 기존 DB가 이름 변경되거나 복사되지는 않습니다.
